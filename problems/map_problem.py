...
class MapProblem(GraphProblem):
    ...
    def set_additional_history_based_data(self):
        # TODO [Ex.26]: Load additional history-based data and assign it to a class variable: (1) Load the csv file
        #  history_4_days_target_{target_id}.csv to a pd.DataFrame (pandas dataframe). Its parent folder is
        #  framework/db/ which is the same as the parent folder of shortest_paths.csv. Use the code in
        #  self.set_additional_shortest_paths_based_data() for help! Each column contains data of a single day,
        #  and each row contains data of a single source: the data is the path cost from each source (each row) to
        #  target_id, based on current_speed on that day (each col). (2) Compute the mean for each source over the 4
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        for link in junction.outgoing_links:
            if self.cost_func_name == 'distance':
                ...
                yield OperatorResult(successor_state=MapState(link.target), operator_cost=link.compute_scheduled_time())
            else:
                yield OperatorResult(successor_state=MapState(link.target), operator_cost=link.compute_current_time())
